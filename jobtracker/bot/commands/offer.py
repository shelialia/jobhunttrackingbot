...
async def offer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("Usage: /offer <app_number>\n\nRun /applied to see the numbered list.")
        return
    ...
    context.user_data["pending_action"] = {"action": "offer", "task_id": task["id"], "display": display}

    await update.message.reply_text(
        f"🎉 Mark *{company}*{role_str} as an *offer*?\n\nSend /confirm to proceed.",
        parse_mode="Markdown",
    )
